...
get_binned_query_series = function(min=0, finish=10, increment=0.5){
  ...
  while (round(min+increment, digits=4) <= finish) {
    # Format query
    queryString = paste(c(preStringCases, round(min, digits=4), preStringControls, round(min, digits=4), postString), collapse = '')
...
get_file_names = function(min=0, finish=10, increment=0.5){ 
  ...
  while (round(min+increment, digits=4) <= finish) {
    # Create curFileName
    curFileName = paste(c(round(min, digits=4), '_', FILTER_PARAM, '_Default_bgl.tsv'), collapse = '')
...
get_greaterThanOrEqualTo_vals = function(min=0, finish=10, increment=0.5){
  ...
  while (round(min+increment, digits=4) <= finish) {
    ...
    vals = append(vals, round(min, digits=4))
    min = min + increment
  }
  # Return vals
  return(vals)
...
get_lessThan_vals = function(min=0, finish=10, increment=0.5){
  ...
  while (round(min+increment, digits=4) <= finish) {
    ...
    vals = append(vals, round(min + increment, digits=4))
    min = min + increment
  }
  # Return vals
  return(vals)
...
write.table(allResults, file=paste(c(as.character(Sys.Date()), "_", FILTER_PARAM, "_Default_", MIN, "-", FINISH, "_increment_", INCREMENT, "_bgl", ".tsv"), collapse = ''), sep="\t", quote=FALSE, row.names=FALSE)
