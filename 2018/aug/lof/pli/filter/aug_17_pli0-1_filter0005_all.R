...
get_file_names = function(min=0, finish=10, increment=0.5){ 
  ...
  while (round(min+increment, digits=4) <= finish) {
    # Create curFileName
    curFileName = paste(c(round(min, digits=4), '_', FILTER_PARAM, '_Default_all.tsv'), collapse = '')
...
write.table(allResults, file=paste(c(as.character(Sys.Date()), "_", FILTER_PARAM, "_Default_", MIN, "-", FINISH, "_increment_", INCREMENT, "_all", ".tsv"), collapse = ''), sep="\t", quote=FALSE, row.names=FALSE)
