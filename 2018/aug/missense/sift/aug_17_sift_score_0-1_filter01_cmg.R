...
FILTER_PARAM = "sift_score"
MAX = 1
FINISH = -0.01
INCREMENT = -0.01
SUBDIR_NAME = paste(c("./", as.character(Sys.Date()), "_", FILTER_PARAM, "_Default_", MAX, "-", FINISH, "_increment_", INCREMENT, "_subqueries"), collapse = '')

get_binned_query_series = function(max=10, finish=0, increment=-0.5){
  ...
  while (round(max+increment, digits=3) >= finish) {
    # Format query
    queryString = paste(c(preStringCases, round(max, digits=3), preStringControls, round(max, digits=3), postString), collapse = '')
...
get_file_names = function(max=10, finish=0, increment=-0.5){ 
  # Create empty vector to store file names
  fileNames = c()
  
  while (round(max+increment, digits=3) >= finish) {
    # Create curFileName
    curFileName = paste(c(round(max, digits=3), '_', FILTER_PARAM, '_Default_cmg.tsv'), collapse = '')
...
get_greaterThanOrEqualTo_vals = function(max=10, finish=0, increment=-0.5){
  # Create empty vector to store values
  vals = c()
  
  while (round(max+increment, digits=3) >= finish) {
    # Append max to vector
    vals = append(vals, round(max, digits=3))
    max = max + increment
  }
  # Return vals
  return(vals)
...
get_lessThan_vals = function(max=10, finish=0, increment=-0.5){
  # Create empty vector to store values
  vals = c()
  
  while (round(max+increment, digits=3) >= finish) {
    # Append max to vector
    vals = append(vals, round(max, digits=3) + increment)
    max = max + increment
  }
  # Return vals
  return(vals)
...
# Build query strings
queries = get_binned_query_series(MAX, FINISH, INCREMENT)

# Build file names
fileNames = get_file_names(MAX, FINISH, INCREMENT)

# Get min vals
minVals = get_greaterThanOrEqualTo_vals(MAX, FINISH, INCREMENT)

# Get max vals
maxVals = get_lessThan_vals(MAX, FINISH, INCREMENT)
...
for (query in queries){
  ...
  if (length(curResult$gene_symbol > 0)){
    ...
    # Add extra column to current query (max query val)
...
write.table(allResults, file=paste(c(as.character(Sys.Date()), "_", FILTER_PARAM, "_Default_", MAX, "-", FINISH, "_increment_", INCREMENT, "_cmg", ".tsv"), collapse = ''), sep="\t", quote=FALSE, row.names=FALSE)
