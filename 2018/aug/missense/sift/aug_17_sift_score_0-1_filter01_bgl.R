...
get_file_names = function(max=10, finish=0, increment=-0.5){ 
  ...
  while (round(max+increment, digits=3) >= finish) {
    # Create curFileName
    curFileName = paste(c(round(max, digits=3), '_', FILTER_PARAM, '_Default_bgl.tsv'), collapse = '')
...
write.table(allResults, file=paste(c(as.character(Sys.Date()), "_", FILTER_PARAM, "_Default_", MAX, "-", FINISH, "_increment_", INCREMENT, "_bgl", ".tsv"), collapse = ''), sep="\t", quote=FALSE, row.names=FALSE)
